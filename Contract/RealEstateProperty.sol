...
contract RealEstateProperty is ERC721, ERC721URIStorage, Ownable{
    ...
    constructor(address _token_address) ERC721("Korea Residential Properties", "KRP") {
    ...
    function tokenizeProperty(address payable _current_owner, string memory _locationAddress, string memory _property_type, uint _last_price, string memory uri) public onlyOwner returns(uint) {
        ...
        properties[token_id] =  Property(_current_owner, payable(0), _locationAddress, token_id, _property_type, _last_price, STATE.not_forSale, uri,0,0,0,0);
        return token_id;
    }
    ...
    function sale(address payable _owner, uint token_id, uint _asking_price, string memory _uri) public returns(uint, uint, uint, string memory){
        ...
        properties[token_id].asking_price = _asking_price;
    ...
    function dates_buyer(address _owner, address payable _buyer, uint _deposti_deadline, uint _closing_date, uint _deposit_basis_points, uint token_id) public {
        ...
        properties[token_id].deposit_basis_points = _deposit_basis_points;
    }
    ...
    function earnest_deposit(address payable _buyer, uint token_id, string memory _uri, string memory report_uri)public payable{
        ...
        uint realAmount = properties[token_id].asking_price * properties[token_id].deposit_basis_points / 10000
        //TODO : ERC transfer
        require(ierc20.balanceOf(msg.sender) > realAmount,"Not Sufficient Balance");
        ierc20.transferFrom(msg.sender,address(this),realAmount);
        properties[token_id].deposit_value = realAmount;
    ...
    function returnDeposit(uint token_id) public {
        ...
        if (appraisal_pass == false || inspection_pass == false){
            ...
            ierc20.transfer(properties[token_id].prospective_buyer,properties[token_id].deposit_value);
    ...
    function askingPrice_balance(uint token_id, string memory report_uri) public payable {
        ...
        ierc20.transfer(properties[token_id].prospective_buyer,properties[token_id].deposit_value);
        ierc20.transferFrom(msg.sender, properties[token_id].prospective_buyer,properties[token_id].asking_price -properties[token_id].deposit_value );
